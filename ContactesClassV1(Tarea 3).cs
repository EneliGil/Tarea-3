...
while (runing)
{
    ...
    switch (typeOption)
    {
        ...
        case 6:
            ...
            break;
        default:
            Console.WriteLine("Eso no existe, pero sigue intentando, futuro genio");
            break;
    }
}
...
static void SearchContact(List<int> ids, Dictionary<int, string> names, Dictionary<int, string> lastnames,Dictionary<int, string> addresses, Dictionary<int, string> telephones, Dictionary<int, string> emails,Dictionary<int, int> ages, Dictionary<int, bool> bestFriends)
{
    ...
    Console.WriteLine($"ID: Nombre - Apellido - Teléfono - Email - Edad - Mejor Amigo");
    Console.WriteLine("----------------------------------------------------------");

    foreach (var id in ids)
    {
        if (names[id].IndexOf(search, StringComparison.OrdinalIgnoreCase) >= 0 ||
            lastnames[id].IndexOf(search, StringComparison.OrdinalIgnoreCase) >= 0)
        {
            string isBF = bestFriends[id] ? "Si" : "No";
            Console.WriteLine($"ID: {id} | {names[id]} {lastnames[id]} | Tel: {telephones[id]} | Email: {emails[id]} | Edad: {ages[id]} | Mejor Amigo: {isBF}");
            found = true;
        }
    }
    ...
    if (!found)
    {
        Console.WriteLine($"No se encontró ningún contacto que contenga '{search}'.");
    }
}
